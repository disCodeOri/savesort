...
function syncErrorResponse(error: YouTubeSyncError) {
  const status =
  ...
  try {
    ...
    const progress =
      parsed.data.action === "start"
        ? await startYouTubeSync(user.id)
        : await continueYouTubeSync(user.id, parsed.data.syncId);
    return NextResponse.json(progress);
  } catch (error) {
    if (error instanceof YouTubeSyncError) return syncErrorResponse(error);
